...
```json
  "code-runner.executorMap": {
    "cpp": "/opt/homebrew/bin/g++-15 -std=gnu++17 -O2 -Wall -Wextra -isysroot \"$(xcrun --show-sdk-path)\" -o $fileNameWithoutExt $fileName && ./$fileNameWithoutExt && rm $fileNameWithoutExt"
  },
...
→ `Ctrl+Option+N` で実行し、ターミナルで入力。入力後 `Ctrl+D` で EOF を送信。
